...
const Menu = () => (
  <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-search">
    ...
    <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-slate-100 rounded-lg bg-slate-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-slate-900 md:dark:bg-slate-900 dark:border-slate-700">
      {['Home', 'Menu', 'Order Online', 'About', 'Contact'].map((item) => (
        <li key={item}>
          <a
            href={`/${item.toLowerCase().replace(' ', '-')}`}
            className="block py-2 px-3 text-slate-900 rounded hover:text-amber-500 md:hover:bg-transparent md:hover:text-amber-500 md:p-0 dark:text-amber-200 dark:hover:text-amber-500 md:dark:hover:bg-transparent dark:border-slate-700"
